...
import { Plus, Flame, CheckCircle2, TrendingUp, Calendar, BrainCircuit, X, Trash2, ChevronRight, MessageSquare } from 'lucide-react';
...
// --- Sub-components (Internal to App.tsx for simplicity, but modularly designed) ---
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const savedCompletions = localStorage.getItem('atomic_completions');
  ...
  useEffect(() => {
    localStorage.setItem('atomic_habits', JSON.stringify(habits));
    localStorage.setItem('atomic_completions', JSON.stringify(completions));
  }, [habits, completions]);
  ...
  const toggleCompletion = (habitId: string) => {
    ...
    if (existingIndex > -1) {
      ...
      setCompletions([...completions, { id: crypto.randomUUID(), habitId, date: today }]);
    }
  };
  ...
  return (
    <div className="min-h-screen pb-24 md:pb-8 flex flex-col items-center">
      {/* Header */}
      <header className="w-full max-w-5xl px-6 py-8 flex items-center justify-between sticky top-0 bg-[#09090b]/80 backdrop-blur-md z-40">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-400 font-medium uppercase tracking-widest">1% Better Every Day</p>
          </div>
        </div>
      ...
      <main className="w-full max-w-5xl px-6 space-y-8">
        {/* Stats Summary */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard icon={<TrendingUp className="text-green-400" />} label="Today's Progress" value={`${stats.completedToday}/${stats.totalHabits}`} subValue={`${stats.totalHabits > 0 ? Math.round((stats.completedToday/stats.totalHabits)*100) : 0}% Done`} />
          <StatCard icon={<Flame className="text-orange-400" />} label="Best Streak" value={stats.bestStreak} subValue="Days" />
          <StatCard icon={<Calendar className="text-blue-400" />} label="Total Habits" value={stats.totalHabits} subValue="Active" />
          <StatCard icon={<CheckCircle2 className="text-purple-400" />} label="Current Best" value={stats.currentBestStreak} subValue="Days" />
        </div>
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-bold flex items-center gap-2">
              Daily Rituals
              <span className="text-xs bg-zinc-800 text-zinc-400 px-2 py-0.5 rounded-full font-mono">{habits.length}</span>
            </h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {habits.length === 0 ? (
              <div className="col-span-full py-12 flex flex-col items-center text-zinc-500 border-2 border-dashed border-zinc-800 rounded-2xl">
                <Plus className="mb-2" size={32} />
                <p>No habits yet. Start with something small.</p>
                <button onClick={() => setIsAddModalOpen(true)} className="mt-4 text-indigo-400 font-medium hover:underline text-sm">Add your first habit</button>
              </div>
            ) : (
              habits.map(habit => (
                <HabitCard 
        ...
        <section className="bg-zinc-900/50 border border-zinc-800 rounded-2xl p-6 overflow-x-auto">
          <h2 className="text-lg font-bold mb-6 flex items-center gap-2">
            Consistency Heatmap
            <span className="text-xs text-zinc-500 font-normal">Last 365 days</span>
          </h2>
          <div className="flex gap-[2px] min-w-[800px]">
            {/* Simple GitHub style grid logic */}
              ...
              {heatmapDates.map(date => {
                ...
                return (
                  <div 
                    ...
                    className={`w-[10px] h-[10px] rounded-[1px] ${opacity} transition-colors hover:ring-1 ring-white/20`} 
                  />
                );
              })}
            </div>
          ...
          <div className="mt-4 flex items-center justify-end gap-2 text-[10px] text-zinc-500 font-medium uppercase tracking-tighter">
            ...
            <div className="flex gap-[2px]">
               <div className="w-[8px] h-[8px] rounded-[1px] bg-zinc-800/40" />
               <div className="w-[8px] h-[8px] rounded-[1px] bg-indigo-900/60" />
               <div className="w-[8px] h-[8px] rounded-[1px] bg-indigo-700/80" />
               <div className="w-[8px] h-[8px] rounded-[1px] bg-indigo-500" />
            </div>
            <span>More</span>
          </div>
        </section>
      ...
      {/* Modals */}
      <Modal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} title="New Daily Habit">
        <form onSubmit={addHabit} className="space-y-4">
          <div className="space-y-2">
            ...
            <input 
              type="text" 
              autoFocus
              className="w-full bg-zinc-800 border border-zinc-700 rounded-xl px-4 py-3 focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition-all"
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium text-zinc-400">Color Theme</label>
            <div className="flex gap-3 justify-between">
              {['#ef4444', '#f59e0b', '#10b981', '#3b82f6', '#8b5cf6', '#ec4899'].map(c => (
                <button
                  ...
                  className={`w-10 h-10 rounded-full transition-all ${newHabitColor === c ? 'ring-4 ring-white/20 scale-110' : 'scale-100'}`}
                  style={{ backgroundColor: c }}
                />
              ))}
            </div>
          </div>
          <button type="submit" className="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-4 rounded-xl shadow-lg shadow-indigo-600/20 transition-all active:scale-[0.98]">
            Create Habit
          </button>
        </form>
      </Modal>

      {/* AI Coach Sidebar/Sheet */}
      {isCoachOpen && (
            ...
            <div className="px-6 py-6 border-b border-zinc-800 flex justify-between items-center">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-indigo-400/80 font-medium">Powered by Gemini</p>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              {chatHistory.length === 0 && (
                <div className="h-full flex flex-col items-center justify-center text-center space-y-4 opacity-60">
                  ...
                  <p className="text-sm px-8">Ask me for habit-building tips, or how to break a cycle of inconsistency.</p>
                  <div className="flex flex-wrap justify-center gap-2">
                    {['How to start a workout habit?', 'Why do I break my streaks?', 'Atomic habits summary'].map(prompt => (
                      <button 
                        key={prompt} 
                        onClick={() => setChatInput(prompt)}
                        className="text-xs bg-zinc-800 px-3 py-1.5 rounded-full hover:bg-zinc-700 transition-colors"
                      >
                        {prompt}
                      </button>
                    ))}
            ...
            <div className="p-6 border-t border-zinc-800">
              <div className="relative flex items-center">
                <input 
                  ...
                  className="w-full bg-zinc-800 border border-zinc-700 rounded-2xl pl-4 pr-12 py-3.5 focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
                  value={chatInput}
                ...
                <button 
                  ...
                  className="absolute right-2 p-2 bg-indigo-600 rounded-xl hover:bg-indigo-500 disabled:opacity-50 disabled:bg-zinc-700 transition-all text-white"
                >
...
// Explicitly use React.FC to handle React's special props like 'key' in list renderings
const StatCard: React.FC<{ icon: React.ReactNode; label: string; value: string | number; subValue: string }> = ({ icon, label, value, subValue }) => {
  return (
    <div className="bg-zinc-900/50 border border-zinc-800/80 p-5 rounded-2xl space-y-3 shadow-sm hover:shadow-md transition-all hover:bg-zinc-900/80 group">
      <div className="flex items-center justify-between">
...
// Explicitly use React.FC to handle React's special props like 'key' in list renderings and satisfy TypeScript
const HabitCard: React.FC<{ habit: Habit; stats: HabitStats; onToggle: () => void; onDelete: () => void }> = ({ habit, stats, onToggle, onDelete }) => {
  const [showDelete, setShowDelete] = useState(false);

  return (
    <div 
      className={`relative group bg-zinc-900/60 border ${stats.isCompletedToday ? 'border-zinc-800' : 'border-zinc-800/40'} rounded-2xl p-5 transition-all hover:shadow-xl hover:translate-y-[-2px]`}
    >
      <div className="flex items-start justify-between mb-4">
        <div className="space-y-1">
          <h3 className={`text-lg font-bold transition-all ${stats.isCompletedToday ? 'text-zinc-500 line-through decoration-2' : 'text-white'}`}>
            {habit.name}
          </h3>
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-zinc-500 font-medium">{stats.totalCompletions} total</span>
          </div>
        </div>
        <button 
          onClick={onToggle}
          ...
          style={!stats.isCompletedToday ? { backgroundColor: habit.color, color: 'white', boxShadow: `0 10px 15px -3px ${habit.color}33` } : {}}
        >
      ...
      {showDelete && (
        <div className="absolute inset-0 bg-zinc-950/95 z-10 rounded-2xl flex flex-col items-center justify-center p-6 text-center animate-in fade-in zoom-in-95 duration-200">
          <p className="text-sm font-medium mb-4">Delete this habit?</p>
          <div className="flex gap-2 w-full">
            <button onClick={() => setShowDelete(false)} className="flex-1 bg-zinc-800 py-2 rounded-xl text-xs font-bold">Cancel</button>
            <button onClick={onDelete} className="flex-1 bg-red-500/10 text-red-400 border border-red-500/20 py-2 rounded-xl text-xs font-bold">Delete</button>
          </div>
        </div>
      )}
    </div>
